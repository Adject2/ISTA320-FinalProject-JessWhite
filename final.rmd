...
Build a scatter plot to answer question #2. Map `chol` to `x`, `thalach` to `y`, and `exang` to `color`.

```{r}
# start with data and then
# map aesthetics with ggplot and add
# point geometrics
heart_disease_data %>%
  ggplot(aes(x = chol, y = exang, color = exang)) +
  geom_point()
```
